...
st.markdown("""
    ...
    .stButton > button { padding: 0px 2px !important; font-size: 11px !important; height: 22px !important; border-radius: 2px !important; }
...
with col_r:
    ...
    for i in range(1, 13):
        if up_cols[(i-1) % 4].button(f"#{i}", key=f"up{i}", use_container_width=True): st.session_state.v_up = str(i)
